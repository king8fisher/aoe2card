...
const unitClasses = {
  ...
  12: "Unused",
  13:
    '<abbr title="except Castles and Kreposts">Stone Defense & Harbors</abbr>',
  14: "Wolves etc.",
  15: "All Archers",
  ...
  30: "Camels",
  31:
    '<abbr title="previously used by the Leitis as armor-ignoring attack">Obsolete</abbr>',
  32: "Condottieri",
  33:
    '<abbr title="no unit has this armor class">Gunpowder units secondary projectile attack</abbr>',
  34: "Fishing Ships",
  35: "Mamelukes",
...
class Tree {
  ...
  updateOffsets() {
    ...
    this.offsets.dark_2 = this.offsets.dark_1 + this.element_height +
      element_offset;
    this.offsets.feudal_1 = this.offsets.dark_2 + this.element_height +
      element_offset;
    this.offsets.feudal_2 = this.offsets.feudal_1 + this.element_height +
      element_offset;
    this.offsets.castle_1 = this.offsets.feudal_2 + this.element_height +
      element_offset;
    this.offsets.castle_2 = this.offsets.castle_1 + this.element_height +
      element_offset;
    this.offsets.imperial_1 = this.offsets.castle_2 + this.element_height +
      element_offset;
    this.offsets.imperial_2 = this.offsets.imperial_1 + this.element_height +
      element_offset;
  }
...
class Lane {
  ...
  updatePositions(offsets, element_length) {
    ...
    for (let r of Object.keys(this.rows)) {
      for (let i = 0; i < this.rows[r].length; i++) {
        if (this.rows[r][i].isBuilding()) {
          this.rows[r][i].x = this.x + ((this.width - this.padding) / 2) -
            (this.rows[r][i].width / 2);
        }
      }
    }
    ...
    for (let connection of connections) {
      ...
      let to = connection[1];
      let allConnectionsForFrom = connections.filter((c) =>
        c[0] === from && carets.has(c[0]) && carets.has(c[1])
      );
      let allRelevantTos = allConnectionsForFrom.map((c) => c[1]);
      if (
        carets.has(from) &&
        carets.get(from).x <
          Math.min(allRelevantTos.map((to_) => carets.get(to_).x))
      ) {
        carets.get(from).x = Math.min(
          allRelevantTos.map((to_) => carets.get(to_).x),
        );
      }
      if (
        carets.has(from) &&
        carets.get(from).x >
          Math.max(allRelevantTos.map((to_) => carets.get(to_).x))
      ) {
        console.assert(
          allRelevantTos.length === 1,
          `Overlapping carets: ${allRelevantTos}`,
        );
...
function formatId(string) {
  return string.toString().replace(/\s/g, "_").replace(/\//g, "_")
    .toLowerCase();
}
...
function enable(buildings, units, techs) {
  for (let item of buildings) {
    ...
    SVG("#building_" + formatId(item.id) + "_disabled_gray").attr({
      "opacity": 0,
    });
  }
...
function applySelectedCiv(selectedCiv) {
  enable(selectedCiv.buildings, [...selectedCiv.units, {
    id: UNIQUE_UNIT,
    age: 3,
  }, { id: ELITE_UNIQUE_UNIT, age: 4 }], [...selectedCiv.techs, {
    id: UNIQUE_TECH_1,
    age: 3,
  }, { id: UNIQUE_TECH_2, age: 4 }]);
  unique([
    ...
    if (items[i].length > 10) {
      ...
      if (space !== -1) {
        ...
        if (alternativeSpace !== -1) {
          if (
            Math.abs((item.length / 2) - alternativeSpace) <
              Math.abs((item.length / 2) - space)
          ) {
        ...
        if (hyphen !== -1) {
          ...
          let alternativeHyphen = hyphen + 1 +
            item.slice(hyphen + 1).indexOf("-");
          if (alternativeHyphen !== -1) {
            if (
              Math.abs((item.length / 2) - alternativeHyphen) <
                Math.abs((item.length / 2) - hyphen)
            ) {
...
function unique(ids, monk_prefix) {
  ...
  SVG("#unit_" + formatId(UNIQUE_UNIT) + "_text").text(
    formatName(data.strings[data.data.units[ids[0]].LanguageNameId]),
  );
  SVG("#unit_" + formatId(UNIQUE_UNIT) + "_overlay").data({
    "name": data.strings[data.data.units[ids[0]].LanguageNameId],
    "id": "unit_" + ids[0],
  });
  SVG("#unit_" + formatId(ELITE_UNIQUE_UNIT) + "_text").text(
    formatName(data.strings[data.data.units[ids[1]].LanguageNameId]),
  );
  SVG("#unit_" + formatId(ELITE_UNIQUE_UNIT) + "_overlay").data({
    "name": data.strings[data.data.units[ids[1]].LanguageNameId],
    "id": "unit_" + ids[1],
  });
  SVG("#tech_" + formatId(UNIQUE_TECH_1) + "_text").text(
    formatName(data.strings[data.data.techs[ids[2]].LanguageNameId]),
  );
  SVG("#tech_" + formatId(UNIQUE_TECH_1) + "_overlay").data({
    "name": data.strings[data.data.techs[ids[2]].LanguageNameId],
    "id": "tech_" + ids[2],
  });
  SVG("#tech_" + formatId(UNIQUE_TECH_2) + "_text").text(
    formatName(data.strings[data.data.techs[ids[3]].LanguageNameId]),
  );
  SVG("#tech_" + formatId(UNIQUE_TECH_2) + "_overlay").data({
    "name": data.strings[data.data.techs[ids[3]].LanguageNameId],
    "id": "tech_" + ids[3],
  });
  SVG("#unit_" + formatId(UNIQUE_UNIT) + "_img").load(
    "img/Units/" + formatId(ids[0]) + ".png",
  );
  SVG("#unit_" + formatId(ELITE_UNIQUE_UNIT) + "_img").load(
    "img/Units/" + formatId(ids[1]) + ".png",
  );
  SVG("#unit_" + formatId(MONK) + "_img").load(
    "img/Units/" + monk_prefix + "125.png",
  );
}
...
function getDefaultTree() {
  ...
  castlelane.rows.castle_2.push(
    new Caret(TYPES.UNIQUEUNIT, UNIQUE_UNIT, UNIQUE_UNIT),
  );
  castlelane.rows.castle_2.push(unit(PETARD));
  castlelane.rows.castle_2.push(tech(UNIQUE_TECH_1));
  castlelane.rows.imperial_1.push(
    new Caret(TYPES.UNIQUEUNIT, ELITE_UNIQUE_UNIT, ELITE_UNIQUE_UNIT),
  );
