...
function displayData() {
  ...
  draw.rect(tree.width, row_height).attr({ fill: "#4d3617", opacity: 0.3 })
    .click(hideHelp);
  draw.rect(tree.width, row_height).attr({ fill: "#4d3617", opacity: 0.3 })
    .click(hideHelp).y(row_height * 2);
  ...
  for (let i = 0; i < image_urls.length; i++) {
    ...
    age_image_group
      ...
      .font({
        size: 16,
        weight: "bold",
      }) /* Text-anchor: middle does not work. */
  ...
  for (let lane of tree.lanes) {
    draw.rect(lane.width + 10, tree.height)
      .attr({
        fill: "#ffeeaa",
        "opacity": 0,
        class: lane.caretIds().map((id) => `lane-with-${id}`),
      })
    ...
    for (let r of Object.keys(lane.rows)) {
      ...
      for (let caret of row) {
        ...
        const image_placeholder = item.rect(
          caret.width * 0.6,
          caret.height * 0.6,
        )
        ...
        const earlier_age_image = item.image(
          "img/Ages/" + getShieldForEarlierRow(r),
        )
          .size(caret.width * 0.3, caret.height * 0.3)
          .attr({
            id: caret.id + "_earlier_age_img_" + ageNumber,
            "opacity": 0,
          })
...
function updateCivselectValue() {
  ...
  let capitalisedHash = hash.substring(0, 1).toUpperCase() +
...
function onAdvancedStatsStateUpdate() {
  try {
    localStorage.setItem(
      "showAdvancedStats",
      (!document.getElementById("advanced-stats").open).toString(),
    );
  } catch (e) {
    // pass
  }
...
function loadCiv() {
  ...
  if (selectedCiv in data.civ_helptexts) {
    document.getElementById("civtext").innerHTML =
      data.strings[data.civ_helptexts[selectedCiv]];
    document.getElementById("civlogo").src =
      `./img/Civs/${selectedCiv.toLowerCase()}.png`;
    window.location.hash = selectedCiv;
  } else {
    document.getElementById("civtext").innerHTML = "";
    document.getElementById("civlogo").src =
      document.getElementById("civlogo").dataset.transparent;
  }
  hideHelp();
}
...
function displayHelp(caretId) {
  ...
  let helptextAdvancedStats = document.getElementById(
    "helptext__advanced_stats",
  );
    ...
    switch (singleTrait) {
      ...
      case 4:
        traitdescriptions.push(
          "Builds:&nbsp;" +
            data.strings[data.data["buildings"][traitPiece]["LanguageNameId"]],
        );
        break;
      case 8:
        traitdescriptions.push(
          "Transforms into:&nbsp;" +
            data
              .strings[
                (data.data["buildings"][traitPiece] ||
                  data.data["units"][traitPiece])["LanguageNameId"]
              ],
        );
        break;
      case 16:
        traitdescriptions.push(
          '<abbr title="has auto-scout behaviour if placed at start">Scout Unit</abbr>',
        );
        break;
      default:
...
function getHelpText(name, id, type) {
  ...
  if (
    (type === "UNIT" || type === "UNIQUEUNIT") && id in data.data.unit_upgrades
  ) {
    text = text.replace(
      /<p class="helptext__stats">/,
      '<h3>Upgrade</h3><p class="helptext__upgrade_cost">' +
        cost(data.data.unit_upgrades[id].Cost) +
        " (" + data.data.unit_upgrades[id].ResearchTime +
        's)<p><p class="helptext__stats">',
    );
  }
  ...
  if (meta !== undefined) {
    ...
    stats.push(
      ifDefinedAndGreaterZero(meta.MaxCharge, chargeText(meta.ChargeType)),
    );
    stats.push(
      ifDefinedAndGreaterZero(meta.RechargeRate, "Recharge Rate:&nbsp;"),
    );
    stats.push(
      secondsIfDefined(meta.RechargeDuration, "Recharge Duration:&nbsp;"),
    );
    if (displayAttack) {
      stats.push(
        secondsIfDefined(
          meta.AttackDelaySeconds,
          "Attack Delay:&nbsp;",
          ranged,
        ),
      );
    ...
    stats.push(
      accuracyIfDefined(meta.AccuracyPercent, "Accuracy:&nbsp;", ranged),
    );
  ...
  for (let civ of Object.keys(data.civ_names)) {
    ...
    if (civs[civ]) {
      if (type === "UNIT" || type === "UNIQUEUNIT") {
        ...
        if (
          civs[civ].buildings.map((item) => `building_${item.id}`).includes(id)
        ) {
          found = true;
        }
      }
...
function cost(cost_object) {
  ...
  if ("Food" in cost_object) {
    value +=
      `<span class="cost food" title="${cost_object.Food} Food">${cost_object.Food}</span>`;
  }
  if ("Wood" in cost_object) {
    value +=
      `<span class="cost wood" title="${cost_object.Wood} Wood">${cost_object.Wood}</span>`;
  }
  if ("Gold" in cost_object) {
    value +=
      `<span class="cost gold" title="${cost_object.Gold} Gold">${cost_object.Gold}</span>`;
  }
  if ("Stone" in cost_object) {
    value +=
      `<span class="cost stone" title="${cost_object.Stone} Stone">${cost_object.Stone}</span>`;
  }
  return value;
}
...
function create_colour_key() {
  ...
  for (let index in LEGEND) {
    ...
    td_type.innerText =
      data.strings[data.tech_tree_strings[LEGEND[index]["name"]]];
  ...
  document.getElementById("key__label").innerText =
    data.strings[data.tech_tree_strings["Key"]];
}
...
function civ(name) {
  ...
  SVG.find(".cross").each(function () {
    ...
    makeSVGObjectOpaque(
      SVG("#" + this.id().replace("_x", "_disabled_gray")),
      0.2,
    );
  });
  ...
  SVG.find(".earlier-age").each(function () {
    ...
    if (type === "unit") {
      if (
        selectedCiv.units.some((item) => item.id === id && item.age === ageId)
      ) {
        makeSVGObjectOpaque(this);
        return;
      }
      ...
      if (
        selectedCiv.buildings.some((item) =>
          item.id === id && item.age === ageId
        )
      ) {
        makeSVGObjectOpaque(this);
        return;
      ...
      if (
        selectedCiv.techs.some((item) => item.id === id && item.age === ageId)
      ) {
        makeSVGObjectOpaque(this);
        return;
      }
...
function main() {
  ...
  techtreeElement.addEventListener("wheel", function (e) {
    ...
    if (
      doVerticalScroll && techtreeDoesNotHaveScrollbar() &&
      shiftKeyIsNotPressed(e)
    ) {
      if (e.deltaY > 0) {
